...
<div class="table-responsive-sm table-responsive-md">
       <table border="0" cellpadding="0" cellspacing="0" class="table table-bordered  table-responsive-sm  month">
        ...
        <span style="display: none">{{$total_harambe = 0}}</span> {{-- end of total count variable --}}
        
         @foreach ($matoleo as $toleo)
        ...
        <tr>
            ...
            <td class='font'>
                ...
                <span style="display: none">{{$bahasha_sadaka = 0}}</span>
                 @foreach ($toleo->member_matoleo as $m )
                 @if($g->kigango == $m->family->jumuiya->mtaa->kigango->name)
                <span style="display:none">{{$bahasha_sadaka+= $m->pivot->sadaka }}</span>
                <span style="display:none">{{$total_sadaka+= $m->pivot->sadaka }}</span> @endif @if($loop->last) <span>{{$bahasha_sadaka}}/=</span>
            ...
            <td class='font'>
                ...
                <span style="display:none">{{$total_shukrani+= $m->pivot->shukrani }}</span> @endif @if($loop->last) <span>{{$bahasha_shukrani}}/=</span>
                <span style="display: none">{{$bahasha_shukrani = 0}}
                            </span><br> @endif @endforeach {{-- end bahasha --}}
            </td>
            <td>
    
                @foreach ($toleo->harambee as $harambe) @if($g->kigango == $harambe->kigango)
                @if($harambe->amount==null)
            ...
            <td class=" text-center ">
                ...
                <div style="display: none">{{$harambee += $harambe->amount }} </div>
                <span style="display: none">{{$total_harambe += $harambe->amount }}</span> @endif
                <br> @endif @endforeach
            </td>
