...
export default {
  ...
  methods: {
    check() {
      axios.get('https://bugandoparish.org/api/vigango').then(response=>{
    ...
    getResults(page = 1) {
      this.loading = true;
      axios
        .get("https://bugandoparish.org/api/mitaa?page=" + page)
        .then(response => {
          this.loading = false;
    ...
    storeData() {
      this.loading = true;
      axios
        .post("https://bugandoparish.org/api/mitaa", {
          name: this.name,
      ...
      search_family(){
                // this.loading=true
                    axios.post('https://bugandoparish.org/api/search_street',{
                    name:this.search  
...
// 
    ...
    get_alldata() {
      this.loading = true;
      axios.get("https://bugandoparish.org/api/mitaa").then(response => {
        this.loading = false;
        this.current_page = response.data.data.current_page;
        this.per_page = response.data.data.per_page;
        this.vigango = response.data.data;
    ...
    deleteData() {
      axios.delete("https://bugandoparish.org/api/mitaa/" + this.id).then(response => {
        if (response.data.status == 200) {
          this.$swal(
    ...
    showData(id) {
      axios.get("https://bugandoparish.org/api/mitaa/" + id).then(response => {
        if (response.data.status == 200) {
          this.get_alldata();
    ...
    updateData(id, kigango, mtaa) {
      ...
      // sdfsdf
      axios
        .put("https://bugandoparish.org/api/mtaa_update", data)
        .then(response => {
          if (response.data.status == 200) {
            this.$swal("Good job!", response.data.message, "success");
            this.get_alldata();
