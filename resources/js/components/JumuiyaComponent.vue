...
import columnSortable from 'vue-column-sortable'
    export default {
      ...
      this.communities.data.sort(fn);
            ...
            getResults(page = 1) {
                 this.loading=true
			axios.get('https://bugandoparish.org/api/jumuiya_retreive?page=' + page)
				.then(response => {
                     this.loading=false
            ...
            storeData(){
                   this.loading=true
                    axios.post('https://bugandoparish.org/api/jumuiya_insert',{
                        name:this.name,
            ...
            get_alldata(){
                this.loading=true
                  axios.get('https://bugandoparish.org/api/jumuiya_retreive')
                   .then(response => {
                        this.loading=false
                     this.current_page=response.data.data.current_page
                       this.per_page=response.data.data.per_page
                        ...
                        this.$swal.fire({
                               ...
                               confirmButtonText: 'Yes, delete it!'
                                     ...
                                     if (result.value) {
                                              ...
                                              axios.delete('https://bugandoparish.org/api/jumuiya_delete/'+id).then(
                                                 response=>{
                                                             if(response.data.status==200)
                                                                       {
                                                                        this.get_alldata();
                     ...
                     axios.get('https://bugandoparish.org/api/jumuiya_show/'+id).then(
                    response=>{
                        if(response.data.status==200){
                ...
                updateData(id,mtaa,jumuiya){
                   var data ={
                      ...
                      axios.put('https://bugandoparish.org/api/jumuiya_update',data).then(
                    response=>{
                        if(response.data.status==200){
                             this.$swal("Good job!", response.data.message, "success");
                                 this.get_alldata();
                ...
                search_community(){
                // this.loading=true
                    axios.post('https://bugandoparish.org/api/jumuiya_search',{
                    name:this.search  
