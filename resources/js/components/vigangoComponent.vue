...
import columnSortable from 'vue-column-sortable'
    export default {
       ...
       methods:{
             ...
             check(){
                      axios.get('http://localhost:8000/api/parokia')
      ...
      this.vigango.data.sort(fn);
            ...
            getResults(page = 1) {
                 this.loading=true
			axios.get('http://localhost:8000/api/vigango?page=' + page)
				.then(response => {
                     this.loading=false
            ...
            search_function(){
                      axios.post('http://localhost:8000/api/kigango_search',{
                        data:this.query
                      }).then(response=>{
                        if(response.data.status == 200){
            ...
            deleteData(){
                ...
                updateData(id,parokia,kigango){
                   ...
                   console.log(data);
                      ...
                      axios.put('http://localhost:8000/api/kigango_search',data).then(
                    response=>{
                        if(response.data.status==200){
                             this.$swal("Good job!", response.data.message, "success");
                                 this.get_alldata();
