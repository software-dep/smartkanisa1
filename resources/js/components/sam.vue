                    ...
                    <table  class="table table-hover table-bordered ">
                     <thead class="table-active">
                        <tr>
                            ...
                            <tbody >
                            <tr v-for="(member,index) in  search_data" :key="index" >
                                <td>{{(index+1)+((current_page-1)*per_page)}}.</td>
                                <td class="text-capitalize">{{member.name}}</td>
                                  <td class="text-capitalize">{{member.family.name}}</td>
                                <td class="text-capitalize">{{member.number}}</td>
...
<div class="tile" style="background-color:rgba(0, 0, 0, 0.09);"><center> <h5>baptized information</h5></center></div>
                                                       ...
                                                       <div class="row" v-if="edit_mode==true">
                                                         ...
                                                         <!-- komunyo -->
<div class="tile" style="background-color:rgba(0, 0, 0, 0.09);"><center> <h5>Communion information</h5></center></div>
                                                        <div class="row"  v-if="edit_mode==false">
                                                       ...
                                                       <div class="row" v-if="edit_mode==true">
                                                           ...
                                                           <!-- kipaimara -->
                                                           <div class="tile" style="background-color:rgba(0, 0, 0, 0.09);"><center> <h5>Confimation information</h5></center></div>
                                                        <div class="row"  v-if="edit_mode==false">
                                                       ...
                                                       <!-- ndoa -->
                                                       <div class="tile" style="background-color:rgba(0, 0, 0, 0.09);"><center> <h5>Marriage information</h5></center></div>
                                                        <div class="row"  v-if="edit_mode==false">
                        ...
                        <tr>
                            ...
                            <tr v-for="(m,index) in search_member" :key="index" >
                                ...
                                <td class="text-capitalize">{{m.name}}</td>
                                  <td class="text-capitalize">{{m.number}}</td>
                                <td class="text-capitalize">{{m.baba}}</td>
                                <td class="text-capitalize">{{m.mama}}</td>
                                ...
                                <td>
                                  <div class="row">
                                    ...
                                    <!-- <div class="col-md-4">  <a data-toggle="modal" @click="edit_details(m.id)" data-target="#more" class='text-success bs-popover-auto' title="click to edit"><span class="fa fa-pencil "></span></a></div>
                                    <div class="col-md-4"> <a data-toggle="modal" @click="status(m.id)" data-target="#edit" class='text-primary bs-popover-auto' title="click to activate or diactivate"><span class="fa fa-eye "></span></a></div> -->
                                    <div class="col-md-4"><a   @click="DeleteMember(m.id)" class="text-danger bs-popover-auto" title="click to delete"> <span class="fa fa-trash "></span></a></div>
                                  </div>
                                </td>
                            </tr>
                        </tbody>
...
import columnSortable from 'vue-column-sortable'
    export default {
      ...
      this.members.data.sort(fn);
            ...
            get_alldata(){
                   this.loading=true
                  axios.get('https://bugandoparish.org/api/members')
                   .then(response => {
                         this.loading=false
                          this.current_page=response.data.data.current_page
                       this.per_page=response.data.data.per_page
             ...
             search_function(){
                //   if(this.selected == 1){
                //    axios.post('http://127.0.0.1:8000/api/vigango',{
                    //  data:this.query
                //    }).then(response =>{
                    //    if(response.data.status == 200){
                            //    this.search_data=response.data.data;
...
// 
                            // }else{
                                //   this.$swal("Oops!", "no family with that name", "error")
// 
                            // }
                //    })
// 
                   ...
                   this.$swal.fire({
                               ...
                               confirmButtonText: 'Yes, delete it!'
                                     ...
                                     if (result.value) {
                                              ...
                                              axios.delete('https://bugandoparish.org/api/members/'+id).then(
                                                 response=>{
                                                             if(response.data.status==200)
                                                                       {
       ...
       this.loading=true
                axios.delete('https://bugandoparish.org/api/members/'+this.id).then(
                    response=>{
                        if(response.data.status==200){
                              this.loading=false
                ...
                updateData(id,name,baba,mama,gender,mahali,tarehe_kuzaliwa,mji,tarehe_kubatizwa,no_ubatizo,msimamizi_ubatizo,parokia_ubatizo,komunyo_ya_kwanza,komunyo_parish,kipaimara,no_kipaimara,parokia_kipaimara,partiner,parokia_ndoa,tarehe_ndoa,no_ndoa){
                   var data ={
                     ...
                     this.loading=true
                      ...
                      axios.put('https://bugandoparish.org/api/update_member',data).then(
                    response=>{
                        if(response.data.status==200){
                              this.loading=false
                ...
                more_details(id){
                     ...
                     axios.get('https://bugandoparish.org/api/members/'+id).then(
                    response=>{
                        if(response.data.status==200){
                ...
                ,status(id){
                      axios.put('https://bugandoparish.org/api/status',{id}).then(
                    response=>{
                            if(response.data.status==200){
                                 this.getResults();
                               this.$swal("Good job!", response.data.message, "success");
                ...
                closebtn(){
                    ...
                    this.fam_member=[];
                    this.query = '';
                },get_family_idget_family_id(name){
                  if(this.selected==1){
                       axios.post('http://localhost:8000/api/search_family_members',{
                          id:name.id
                  })
